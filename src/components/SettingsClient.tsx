...
export function SettingsClient({ settings, masterResumes, hasCareerProfile, integration, notificationsEnabled }: { settings: SearchSettings; masterResumes: MasterResume[]; hasCareerProfile: boolean; integration: Record<string, boolean>; notificationsEnabled: boolean }) {
  ...
  function uploadResume(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      setMessage(res.ok ? 'Updated master resume added to Google Drive and set as the active baseline.' : data.error ?? 'Resume update failed.');
  ...
  return <div className="detail-layout">
    ...
    <section className="panel"><h2>Update baseline resume</h2><p className="fit">Add a newer baseline resume at any time using a DOCX file, PDF file, or native Google Doc. The original remains in Drive, the new version is added to the master resume library, and the selected baseline becomes the source for future analysis and tailoring until you change it.</p><ol className="settings-steps"><li>Enter a clear resume label and select the primary search lane.</li><li>Choose one source: upload a DOCX or PDF file, or paste the URL of a native Google Doc that your connected Google account can access.</li><li>Select <strong>Add and use as baseline</strong>. The app copies the new version into the master career documents folder, sets it as the active baseline, and refreshes the Career Evidence Profile.</li><li>Use the Master resume library below whenever you want to switch the active baseline to another saved version.</li></ol><form className="form" onSubmit={uploadResume} style={{marginTop:12}}><div className="field"><label htmlFor="name">Resume label</label><input id="name" name="name" defaultValue="Current Master Resume" required/></div><div className="field"><label htmlFor="lane">Primary search lane</label><select id="lane" name="lane" defaultValue="PROGRAM_PROJECT"><option value="PROGRAM_PROJECT">Project + Program</option><option value="EXECUTIVE">Executive</option><option value="AGILE">Agile</option><option value="PRODUCT">Product</option></select></div><div className="field"><label htmlFor="resume">DOCX or PDF file</label><input id="resume" name="resume" type="file" accept=".docx,.pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/pdf"/><span className="field-help">Maximum file size: 10 MB. Leave blank when using a Google Docs URL.</span></div><div className="source-divider"><span>or</span></div><div className="field"><label htmlFor="googleDocUrl">Google Docs URL</label><input id="googleDocUrl" name="googleDocUrl" type="url" placeholder="https://docs.google.com/document/d/..."/><span className="field-help">Use a native Google Doc. Leave blank when uploading a DOCX or PDF file.</span></div><button className="button primary" disabled={pending}>Add and use as baseline</button></form></section>
