...
const ROTATING_VALUE = [
  ...
  "Review jobs before you apply.",
  "Create tailored resumes and cover letters.",
  "Keep your entire search organized.",
];

const SYSTEM_ACTIVITY = [
  "Looking for strong job matches",
  "Checking salary and work location",
  "Comparing jobs with your experience",
      ...
      <header className={styles.header}>
        <div className={styles.brand}>
          ...
          <div>
            ...
            <span>Private career workspace</span>
          </div>
        </div>
        <div className={styles.secureBadge}><ShieldCheck size={15} /> Private & secure</div>
      </header>

      <main className={styles.main}>
        <section className={styles.copyColumn}>
          <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className={styles.eyebrow}>
            <span className={styles.liveDot} />
            <BrainCircuit size={15} />
            Your private AI-powered job search workspace
          </motion.div>
          ...
          <div className={styles.rotatingLine} aria-live="polite">
            <AnimatePresence mode="wait"><motion.p key={valueIndex} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.35 }}>{ROTATING_VALUE[valueIndex]}</motion.p></AnimatePresence>
          </div>

          <motion.p initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className={styles.subcopy}>
            Find jobs that fit your experience, understand which ones are worth pursuing, create tailored application materials, and keep your progress organized in one private workspace.
          </motion.p>

          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.28 }} className={styles.actions}>
            <a className={styles.loginButton} href="/api/auth/google/start"><span className={styles.googleMark}>G</span><span>Sign in with Google</span><ArrowRight size={17} /></a>
            <div className={styles.securityCopy}><LockKeyhole size={14} /><span>Only the approved Google account can open this workspace.</span></div>
          </motion.div>

          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.42 }} className={styles.capabilities}>
            <span><Search size={15} /> Find jobs</span>
            <span><Target size={15} /> Compare matches</span>
            <span><FileCheck2 size={15} /> Prepare application materials</span>
            <span><BriefcaseBusiness size={15} /> Track applications</span>
          </motion.div>
        </section>

        <motion.section initial={{ opacity: 0, x: 28 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.75, delay: 0.18 }} className={styles.previewColumn} aria-label="Example of what the app can show after you sign in">
          <div className={styles.previewShell}>
            ...
            <div className={styles.activityBar}><span className={styles.scanDot} /><AnimatePresence mode="wait"><motion.span key={activityIndex} initial={{ opacity: 0, y: 7 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -7 }} transition={{ duration: 0.25 }}>{SYSTEM_ACTIVITY[activityIndex]}...</motion.span></AnimatePresence></div>
          </div>
          <div className={styles.floatingCardOne}><span>Applications ready</span><strong>3</strong></div>
          <div className={styles.floatingCardTwo}><span>Strong matches</span><strong>7</strong></div>
        </motion.section>
      </main>
