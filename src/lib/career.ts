...
async function masterFolderId(): Promise<string> {
  ...
  if (!folder?.id) throw new Error('Google Drive master career documents folder was not found.');
  return folder.id;
}
...
export async function syncCareerProfile() {
  ...
  if (!sources.length) throw new Error('Master resume files were found but none could be parsed.');
