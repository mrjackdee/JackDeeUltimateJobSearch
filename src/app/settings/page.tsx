...
import { supportDatabaseConfigured } from '@/lib/support';
import { SettingsClient } from '@/components/SettingsClient';
import { TargetCompaniesClient } from '@/components/TargetCompaniesClient';
...
export default async function SettingsPage() {
  ...
  const integration = {
    ...
    'Support ticket database': supportDatabaseConfigured(),
  ...
  return <><section className="hero"><div className="eyebrow">Settings</div><h1>Control how the app searches and which resume it uses.</h1><p>Update your resume, refresh your Career Profile, manage job matching preferences, and review the services the app needs to work properly.</p><div className="hero-actions"><Link className="button" href="/admin">Admin & Diagnostics</Link><Link className="button" href="/support">Support</Link><Link className="button" href="/guide">How to use the app</Link></div></section><SettingsClient settings={state.settings} masterResumes={state.masterResumes} hasCareerProfile={Boolean(state.careerProfile)} integration={integration} notificationsEnabled={state.notificationsEnabled !== false}/><div style={{marginTop:14}}><TargetCompaniesClient companies={state.settings.targetCompanies ?? []}/></div></>;
}
